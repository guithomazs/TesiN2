...
class Players:
    ...
    def getPlayerData(self):
        player = self.players_treeview.selection()
        if len(player) > 1:
            messagebox.showwarning('Aviso.', 'Apenas escolha o seu jogador por favor.')
            return
        elif len(player) != 1:
            messagebox.showwarning('Aviso.', 'É necessário ao menos um jogador para mostrar as informações.')
            return
        nick_user = self.players_treeview.item(player, 'text')
        PlayerInfo(self.root, nick_user, root_players_treeview=self.players_treeview, chosen_player=player)
